...
import { useClassesStore, type StoredClass } from "@/lib/classes-store";

export default function MisClasesPage() {
  const { classes, loaded, createClass } = useClassesStore();
  const [open, setOpen] = React.useState(false);
  const [name, setName] = React.useState("");
  const [subject, setSubject] = React.useState("");
  const [description, setDescription] = React.useState("");
  const [copied, setCopied] = React.useState(false);
  const [created, setCreated] = React.useState<StoredClass | null>(null);
  ...
  function handleCreate(e: React.FormEvent) {
    e.preventDefault();
    const newClass = createClass({ name, subject, description });
    setCreated(newClass);
  }
        ...
        {loaded &&
          classes.map((c, i) => (
            <motion.div
              ...
              whileHover={{ y: -3 }}
            >
              <Link href={`/docente/clases/${c.id}`} className="block">
                <Card className="cursor-pointer p-6 transition-shadow hover:shadow-[var(--shadow-soft-lg)]">
                  ...
                  <div className="mt-5 flex items-center justify-between text-sm">
                    ...
                    <div className="flex items-center gap-1 font-semibold" style={{ color: c.color }}>
                      Promedio {c.average}%
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="Nueva Clase" description="Crea un nuevo grupo para tus alumnos.">
        {created ? (
          <div className="flex flex-col items-center gap-4 py-4 text-center">
            ...
            <div>
              <p className="font-semibold">¡{created.name} creada!</p>
          ...
          <form onSubmit={handleCreate} className="flex flex-col gap-4">
            ...
            <div className="rounded-xl bg-[var(--color-surface-muted)] px-4 py-3 text-sm text-black/50">
              El código de la clase se genera automáticamente al crearla.
            </div>
